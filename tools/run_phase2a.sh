...

{
  ...
  docker exec -w /workspace/altgestures codex-cli \
    ...
    "读取仓库根目录 AGENTS.md 与 specs/PHASE2A-SPEC.md，严格按 SPEC 全部规格实现 Phase 2A（Windows 互操作基础层：P/Invoke 声明 + 输入模拟 + 显示器信息）。关键约束：目标框架必须保持 net8.0 且必须在当前 Linux 环境编译通过，禁止改成 net8.0-windows、禁止引入 CsWin32、禁止引用 System.Windows.Forms。禁止修改 reference/ 与 specs/ 目录；禁止翻译 reference/altdrag 的 C 代码。完成后按 SPEC 第 12 节填写交付检查清单，并附真实命令输出。"
  rc=$?
